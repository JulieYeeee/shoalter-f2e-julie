...
const searchResultSelector = createSelector(
    ...
    (result, keyword) => {
        console.log("keyword itself:", keyword)
        if (keyword) {
            const filterByName = result.entry.filter((item) => {
                return item["im:name"].label.toLowerCase().includes(keyword.toLowerCase())
            })
            ...
            // 去除重複
            const resultSet = new Set([...filterByName, ...filterBySummary, ...filterByTitle]);
...
export { getSearchResultThunk, getRecommendationThunk, searchResultSelector, recommendationSelector }
